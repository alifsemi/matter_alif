...
def validate_tools(args):
    ...
    TOOLS['xxd'] = shutil.which('xxd')

    if TOOLS['xxd'] is None:
        logger.error('xxd not found, please install xxd')
        sys.exit(1)
    

    logger.info('Using following tools:')
    logger.info('spake2p:    {}'.format(TOOLS['spake2p']))
    logger.info('chip-cert:  {}'.format(TOOLS['chip-cert']))
    logger.info('chip-tool:  {}'.format(TOOLS['chip-tool']))
    logger.info('xxd:  {}'.format(TOOLS['xxd']))
...
def setup_out_dir(out_dir_top, serial: str):
    os.makedirs(out_dir_top, exist_ok=True)
    dirs = {
        'output': out_dir_top,
        'internal': os.sep.join([out_dir_top, 'internal']),
    }
...
def generate_fd_binary(partition_offset, partition_size, hex_file, bin_file):
    logger.info('Generating Factory data partition image: offset: 0x{:X} size: 0x{:X}'.format(partition_offset, partition_size))
    cbor_data = cbor.dumps(NVS_MEMORY)

    # Create hex file
    if len(cbor_data) > partition_size:
        raise ValueError("generated CBOR file exceeds declared maximum partition size! {} > {}".format(len(cbor_data), partition_size))
    ih = IntelHex()
    # Write CBOR data
    ih.putsz(partition_offset, cbor_data)
    ih.write_hex_file(hex_file, True)
    ih.tobinfile(bin_file)
    logger.info("Generated factory data")
    logger.info("Hex: {}".format(hex_file))
    logger.info("Bin: {}".format(bin_file))

def generate_header(header_out, bin_file):
    # Copy original bin file for generate stable header variable names
    shutil.copy(bin_file, "factory_data.bin")
    logger.info("Generate factory data to: {}".format(header_out))
...
def generate_arg_parser():
    ...
    parser = argparse.ArgumentParser(description='Factory data generation test tool',
                                     formatter_class=lambda prog: argparse.HelpFormatter(prog, max_help_position=50))

    # Main options
    main_args = parser.add_argument_group('Main options')
    main_args.add_argument("--output", type=str, required=False, default="out",
                              help="[string] Output path where generated data will be stored.")
    main_args.add_argument("--spake2-path", type=str, required=False,
                              help="[string] Provide Spake2+ tool path")
    main_args.add_argument("--chip-tool-path", type=str, required=False,
                              help="[string] Provide chip-tool path")
    main_args.add_argument("--chip-cert-path", type=str, required=False,
                              help="[string] Provide chip-cert path")
    main_args.add_argument("--enable-key", type=str,
    ...
    com_args = parser.add_argument_group('Commisioning options')
    com_args.add_argument('--passcode', type=allow_any_int,
                                    help='The passcode for pairing. Randomly generated if not specified.')
    com_args.add_argument("--spake2-it", type=allow_any_int, default=1000,
                                    help="[int] Provide Spake2+ iteration count.")
    com_args.add_argument('--discriminator', type=allow_any_int,
                                    help='The discriminator for pairing. Randomly generated if not specified.')
    com_args.add_argument('-cf', '--commissioning-flow', type=allow_any_int, default=0,
                                    help='Device commissioning flow, 0:Standard, 1:User-Intent, 2:Custom. \
                                    Default is 0.', choices=[0, 1, 2])
    com_args.add_argument('-dm', '--discovery-mode', type=allow_any_int, default=1,
                                    help='Commissionable device discovery networking technology. \
                                          0:WiFi-SoftAP, 1:BLE, 2:On-network. Default is BLE.', choices=[0, 1, 2])

    # Device information
    dev_info_args = parser.add_argument_group('Device instance information options')
    dev_info_args.add_argument('-v', '--vendor-id', type=allow_any_int, required=False, help='Vendor id')
    dev_info_args.add_argument('--vendor-name', type=str, required=False, help='Vendor name')
    dev_info_args.add_argument('-p', '--product-id', type=allow_any_int, required=False, help='Product id')
    dev_info_args.add_argument('--product-name', type=str, required=False, help='Product name')
    dev_info_args.add_argument('--hw-ver', type=allow_any_int, required=False, help='Hardware version')
    dev_info_args.add_argument('--hw-ver-str', type=str, required=False, help='Hardware version string')
    dev_info_args.add_argument('--mfg-date', type=str, required=False, help='Manufacturing date in format YYYY-MM-DD')
    dev_info_args.add_argument('--serial-num', type=str, required=False, help='Serial number in hex format')
    dev_info_args.add_argument('--enable-rotating-device-id', action='store_true',
                               help='Enable Rotating device id in the generated binaries')
    dev_info_args.add_argument('--rd-id-uid', type=str, required=False,
    ...
    part_gen_args.add_argument('--offset', type=allow_any_int, required=True,
                               help='Partition offset - an address in devices NVM memory, where factory data will be stored')
    part_gen_args.add_argument('--size', type=allow_any_int, required=True, help='The maximum partition size')
    return parser


def get_and_validate_args(parser):
...
def main():
    ...
    args = get_and_validate_args(parser)
    # Validate that all tools are exist
    validate_tools(args)
    ...
    pai_cert = {}
    serial_num_str = format(serial_num_int , 'x')
    logger.info("Generating for {}".format(serial_num_str))
    out_dirs = setup_out_dir(out_dir_top, serial_num_str)
    ...
    hex_file = os.sep.join([out_dirs['output'], 'factory_data.hex'])
    bin_file = os.sep.join([out_dirs['output'], 'factory_data.bin'])
    header_out = os.sep.join([out_dirs['output'], 'internal/factory_data_base.h'])
    generate_fd_binary(args.offset, args.size, hex_file, bin_file)
    generate_header(header_out, bin_file)
