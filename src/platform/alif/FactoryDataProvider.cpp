...
CHIP_ERROR MatterFactoryDataInit(void)
{
    int ret;
    uint8_t flash_slot[16], temp_slot[16];
    const struct device * flashDevice;
    off_t factoryDataOffset = FIXED_PARTITION_OFFSET(factory_partition);
    size_t factoryDataSize  = FIXED_PARTITION_SIZE(factory_partition);
    size_t factoryDataPageAligment = factory_data_bin_len % 16;
    size_t len = factory_data_bin_len - factoryDataPageAligment;

    const uint8_t * data_ptr = factory_data_bin;

    flashDevice = DEVICE_DT_GET(DT_CHOSEN(zephyr_flash_controller));

    memset(flash_slot, 0, 16);

    ret = flash_read(flashDevice, factoryDataOffset, temp_slot, 16);
    if (ret != 0)
    {
        ChipLogError(DeviceLayer, "Factory Data partion read fail (%d)", ret);
        return CHIP_ERROR_READ_FAILED;
    }

    if (memcmp(temp_slot, flash_slot, 16) != 0)
    {
        return CHIP_NO_ERROR;
    }

    ret = flash_erase(flashDevice, factoryDataOffset, factoryDataSize);
    if (ret != 0)
    {
        ChipLogError(DeviceLayer, "Factory Data partion Erase fail (%d)", ret);
        return CHIP_ERROR_READ_FAILED;
    }

    ret = flash_write(flashDevice, factoryDataOffset, (void *) data_ptr, len);
    if (ret != 0)
    {
        ChipLogError(DeviceLayer, "Factory Data partion data write fail (%d)", ret);
        return CHIP_ERROR_WRITE_FAILED;
    }

    if (factoryDataPageAligment)
    {
        data_ptr += len;
        factoryDataOffset += len;
        
        memset(temp_slot, 0, 16);
        memcpy(flash_slot, data_ptr, factoryDataPageAligment);
        /* Read the factory data header from flash */
        ret = flash_write(flashDevice, factoryDataOffset, (void *) flash_slot, 16);
        if (ret != 0)
        {
            ChipLogError(DeviceLayer, "Factory Data partion data aligment part write fail (%d)", ret);
            return CHIP_ERROR_WRITE_FAILED;
        }
    }
...
CHIP_ERROR FactoryDataProvider<FlashFactoryData>::Init()
{
    ...
    error = MatterFactoryDataInit();
    if (error != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer, "Failed to Init factory init data");
        return error;
    }
